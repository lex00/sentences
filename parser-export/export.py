...
class W(torch.nn.Module):
    ...
    def forward(s, input_ids, attention_mask, words_from_tokens, decoder_input_ids, decoder_attention_mask, valid_token_mask):
        ss,ts = s.m.forward(dict(input_ids=input_ids, attention_mask=attention_mask, words_from_tokens=words_from_tokens,
            decoder_input_ids=decoder_input_ids, decoder_attention_mask=decoder_attention_mask, valid_token_mask=valid_token_mask))
        return ss, ts
...
dyn={k:{0:"b",1:"s"} for k in keys}; dyn["span_scores"]={0:"b",1:"w",2:"w2"}; dyn["tag_scores"]={0:"b",1:"t"}
torch.onnx.export(W(m).eval(), args, "benepar.onnx", opset_version=17, input_names=keys,
    output_names=["span_scores","tag_scores"], dynamic_axes=dyn, do_constant_folding=True, dynamo=False)
