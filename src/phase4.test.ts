...
const segs = (s: Scene, role: string): Extract<Prim, { kind: "seg" }>[] => {
  const out: Extract<Prim, { kind: "seg" }>[] = [];
  (function w(n: SceneNode): void {
    for (const c of n.children) {
      if (!isNode(c) && c.kind === "seg" && c.role === role) out.push(c);
      if (isNode(c)) w(c);
    }
  })(s.root);
  return out;
};
const labels = (s: Scene): string[] => {
  const out: string[] = [];
  (function w(n: SceneNode): void {
    for (const c of n.children) {
      if (!isNode(c) && c.kind === "lbl") out.push(c.text);
      if (isNode(c)) w(c);
    }
  })(s.root);
  return out;
...
describe("subordinate clause (nesting)", () => {
  ...
  it("nests a full clause under the verb with derived ids", () => {
    expect(ids(s)).toEqual(
      expect.arrayContaining(["c/verb/m0", "c/verb/m0/c", "c/verb/m0/c/subj", "c/verb/m0/c/verb"]),
    );
