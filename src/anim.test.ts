...
import { isNode, type Scene } from "./scene.js";
...
const A = (): Scene => layout(irA, metrics);
const B = (): Scene => layout(irB, metrics);

const lblX = (s: Scene, nodeId: string, text: string): number => {
  let x = NaN;
  (function w(n: any): void {
    if (n.id === nodeId) for (const c of n.children) if (!isNode(c) && c.kind === "lbl" && c.text === text) x = c.anchor.x;
