...
export type UnigramModel = { vocab: Array<[string, number]>; unk_id: number; eos_id: number; pad_id: number };

export type Feeds = {
  input_ids: number[];
  attention_mask: number[];
  words_from_tokens: number[];
  decoder_input_ids: number[];
  decoder_attention_mask: number[];
  valid_token_mask: boolean[];
};

const SPACE = "▁"; // ▁ metaspace marker

export class T5Tokenizer {
  private piece = new Map<string, { id: number; score: number }>();
  private maxLen = 1;
  constructor(private model: UnigramModel) {
    model.vocab.forEach(([p, score], id) => {
      this.piece.set(p, { id, score });
      if (p.length > this.maxLen) this.maxLen = p.length;
    });
  }

  // Unigram Viterbi over one word, prefixed with the metaspace marker.
  tokenizeWord(word: string): number[] {
    const s = SPACE + word.normalize("NFKC");
    const n = s.length;
    const best = new Array(n + 1).fill(-Infinity);
    const back = new Array<{ start: number; id: number }>(n + 1);
    best[0] = 0;
    for (let i = 1; i <= n; i++) {
  ...
  buildFeeds(words: string[]): Feeds {
    const subwords: number[] = [];
    const wft: number[] = [0]; // start token -> decoder index 0
