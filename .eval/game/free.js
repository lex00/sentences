...
const wordsOf = (els) => els.filter((e) => e.kind === "word");
// count words whose POS tag starts with any prefix (JJ -> JJ/JJR/JJS, etc.)
const countTag = (a, prefixes) => posTags(a.tree).filter((t) => prefixes.some((p) => t.tag.startsWith(p))).length;
const CHALLENGES = [
    {
...
async function check() {
    ...
    if (!model) {
        ensureModel();
        statusEl.textContent = modelState === "loading" ? "still loading the parser — try again in a moment" : statusEl.textContent;
        return;
    }
