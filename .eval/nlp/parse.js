...
const leaf = (label, word) => ({ label, word, children: [] });
...
const looksLikeVerb = (t) => t.forced === "V" || (t.tag === "X" && (VERBS.has(t.lc) || t.lc.endsWith("ed") || t.lc.endsWith("ing")));
...
const isAux = (t) => t.tag === "MD" || t.tag === "AUX" || t.tag === "COP";
function normalizeQuestion(ts) {
    if (ts.length === 0)
        return ts;
    ...
    if (a0.tag === "X") {
        if (MODAL.has(a0.lc))
            a0.tag = "MD";
        else if (COPULA.has(a0.lc))
            a0.tag = "COP";
        else if (AUX.has(a0.lc))
            a0.tag = "AUX";
    }
    let wh = null;
    let rest = ts;
    ...
    if (rest.length > 0 && isAux(rest[0])) {
        ...
        let k = 1;
        while (k < rest.length && (rest[k].tag === "DT" || rest[k].tag === "PRP$"))
            k++;
        if (k < rest.length && rest[k].tag === "PRP")
            k++;
        else {
            while (k < rest.length && (rest[k].tag === "JJ" || rest[k].tag === "RB"))
                k++; // pre-noun modifiers
            while (k < rest.length && (rest[k].tag === "X" || rest[k].tag === "CD") && !looksLikeVerb(rest[k]))
                k++; // noun(s)
        }
        ...
        if (subj.length > 0) {
            let out = [...subj, aux, ...rest.slice(k)]; // un-invert: subject + aux + rest
            if (wh)
                out = [...out, { word: wh.word, lc: wh.lc, tag: WH_ARG.has(wh.lc) ? "X" : "RB" }]; // arg->object, adjunct->adverb
            return out;
        }
    }
    // no inversion: a wh-subject ("Who chased the cat") stays the subject
    if (wh && WH_ARG.has(wh.lc))
        return [{ word: wh.word, lc: wh.lc, tag: "X" }, ...rest];
    return ts;
}
function finiteVerbTag(word) {
    const lc = word.toLowerCase();
    const cop = { is: "VBZ", are: "VBP", am: "VBP", was: "VBD", were: "VBD", be: "VB", been: "VBN", being: "VBG" };
    const aux = { has: "VBZ", have: "VBP", had: "VBD", do: "VBP", does: "VBZ", did: "VBD" };
    return cop[lc] ?? aux[lc] ?? (lc.endsWith("ing") ? "VBG" : lc.endsWith("ed") ? "VBD" : lc.endsWith("s") ? "VBZ" : "VBP");
}
const nounTag = (word) => (isCapitalized(word) ? "NNP" : word.toLowerCase().endsWith("s") ? "NNS" : "NN");
// determiner/possessive? + (adverb|adjective)* + head noun, OR a bare pronoun.
function parseBaseNP(ts, i, end) {
    const kids = [];
    let j = i;
    if (j < end && ts[j].tag === "DT")
        kids.push(leaf("DT", ts[j++].word));
    else if (j < end && ts[j].tag === "PRP$")
    ...
    while (j < end && (ts[j].tag === "X" || ts[j].tag === "JJ" || ts[j].tag === "CD" || ts[j].tag === "RB")) {
        if (looksLikeVerb(ts[j]) && hasNoun)
            break;
        if (ts[j].tag === "X" || ts[j].tag === "CD")
            hasNoun = true;
        run.push(ts[j]);
        j++;
    }
    if (run.length === 0 && kids.length === 0)
        return null;
    // last open-class word is the head noun; earlier ones are modifiers
    let headIdx = -1;
    for (let k = run.length - 1; k >= 0; k--)
        if (run[k].tag === "X" || run[k].tag === "CD") {
            headIdx = k;
            break;
        }
    if (headIdx === -1)
        headIdx = run.length - 1; // no clear noun (e.g. an -ly word) -> last token is head
    run.forEach((t, k) => {
        if (k === headIdx)
            kids.push(leaf(nounTag(t.word), t.word));
        else if (t.tag === "RB")
            kids.push(leaf("RB", t.word));
        else if (t.tag === "CD")
            kids.push(leaf("CD", t.word));
        else
            kids.push(leaf("JJ", t.word)); // pre-head open-class word -> adjective
    });
    if (headIdx === -1 && kids.length === 0)
        return null;
    return { tree: node("NP", kids), next: j };
}
function parsePP(ts, i, end) {
    if (ts[i].tag !== "IN" && ts[i].tag !== "TO")
        return null;
...
function parseInfinitive(ts, i, end) {
    const verb = ts[i + 1];
    if (!verb)
        return null;
    const kids = [leaf("TO", ts[i].word), leaf("VB", verb.word)];
    let j = i + 2;
...
function parseSingleVP(ts, i, end) {
    const kids = [];
    let j = i;
    let isCop = false;
    const isAuxOrVerb = (t) => t.tag === "MD" || t.tag === "AUX" || t.tag === "COP" || looksLikeVerb(t);
    while (j < end) {
        const t = ts[j];
        if (isAuxOrVerb(t)) {
            if (t.tag === "COP")
    ...
    while (j < end && ts[j].tag !== "CC") {
        ...
        if (t.tag === "RB") {
            ...
            const adjLike = (x) => (x.tag === "JJ" || x.tag === "X") && !looksLikeVerb(x);
            const adj = [];
            for (;;) {
                if (j < end && adjLike(ts[j]))
                    adj.push(leaf("JJ", ts[j++].word));
                else if (j + 1 < end && ts[j].tag === "CC" && adjLike(ts[j + 1]))
                    adj.push(leaf("CC", ts[j++].word));
                else
                    break;
            }
...
function parseVP(ts, i, end) {
    ...
    if (j < end && ts[j].tag === "CC") {
        ...
        if (rhs)
            return { tree: node("VP", [first.tree, leaf("CC", ts[j].word), rhs.tree]), next: rhs.next };
    }
    return first;
}
function parseS(ts, i, end) {
...
export function parse(text) {
    ...
    if (clauses.length === 0)
        throw new Error("couldn't parse: expected a subject and a verb");
    if (clauses.length === 1)
        return clauses[0];
    const kids = [];
    clauses.forEach((c, k) => {
